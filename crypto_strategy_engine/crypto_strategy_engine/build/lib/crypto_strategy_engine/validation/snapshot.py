...
from ..compatibility import normalize_market_snapshot
...
def validate_market_snapshot(
    ...
    *,
    ...
    if not isinstance(snapshot.get("levels"), (Mapping, list)):
        raise ValueError("snapshot:levels_not_object_or_array")
    for name in ("derivatives", "breakout_alert"):
        if snapshot.get(name) is not None and not isinstance(snapshot.get(name), Mapping):
            raise ValueError(f"snapshot:{name}_not_object_or_null")
    ...
    original = json.loads(canonical_json(dict(snapshot)))
    frozen = normalize_market_snapshot(original)
    return SnapshotValidation(
        valid=not blockers,
        frozen=frozen,
        snapshot_hash=canonical_hash(original),
